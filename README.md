...
with confirms, then sets `published_at`. The consumer is idempotent because
`succeed()` / `fail()` are no-ops on a terminal payment. Transient webhook/gateway
errors get 3 attempts with exponential TTL (`1s`, then `2s`) and then land on
...
3. Body (keep `webhook_url` public — the consumer runs in Docker and
   cannot call `localhost` on your machine):
